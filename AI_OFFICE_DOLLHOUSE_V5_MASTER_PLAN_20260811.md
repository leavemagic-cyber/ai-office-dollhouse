...
### D8b. 平面定案（Owner 2026-08-12 三項指正後）
- **開放式獨立桌**：一人一桌共 6 張（對應一層 6 人），不是 bench 連桌。
  （我先前把 bench 講成「真實的解法」是錯的，已更正——那只是歐洲密集辦公室的一種慣例。）
- **每桌自帶屏風**：畫在桌子上的擋板，高 1.42 storey、純外框。
  高 1.78 且有填色的版本實測會把工位區變成亮面板並吃掉人偶，已否決。
- **兩排工位分散**：`bank(1.9,5.0)` 與 `bank(7.0,3.95)`，投影後橫跨螢幕 x 39-97
  （原本兩排的 x 範圍完全重疊在 38-53，這才是「看起來擠」的成因）；行內間距 1.35 格。
- 物件數 26（工作層）／28（單層），較最初的 36 少 8 件。
- Grok 的整體方案 Owner 指示不採用；Codex 的「集中座位、右側留 15 格² 大空地」方案記錄備查未採用。
