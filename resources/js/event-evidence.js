...
// never manufacture an outcome. A user instruction may instead animate its
// literal request when the hook classifies it in memory and persists only the
// action type, never the instruction text.
export const EVENT_EVIDENCE = Object.freeze({
  ...
  delegation_requested: { sources: ['session:delegation_requested', 'hook:intent:delegation_requested', 'orchestration:delegation_requested'], legacyTierA: false },
  coordination_message: { sources: ['session:coordination_message', 'hook:intent:coordination_message', 'orchestration:coordination_message'], legacyTierA: false },
  ...
  // action (for example `review_passed`). That is a real local-record fact. Hook
  // request types above inspect no tool input/output and never promote an instruction
  // into a result.
  handoff_requested: { sources: ['hook:intent:handoff_requested'], legacyTierA: false },
  owner_consult_requested: { sources: ['hook:intent:owner_consult_requested'], legacyTierA: false },
  review_requested: { sources: ['hook:intent:review_requested'], legacyTierA: false },
  ...
  revision_requested: { sources: ['orchestration:revision_requested', 'session:revision_requested', 'hook:intent:revision_requested'], legacyTierA: false },
